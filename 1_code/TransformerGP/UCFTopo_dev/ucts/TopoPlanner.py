...
class TopoGenSimulator(uct.Simulator):
    def __init__(self, _configs, _num_component=4, target=None):
        ...
        if config.use_external_hash:
            try:
                self.all_graph_2_reward = json.load(open('reward_hash.json', 'r'))
                print('hash table loaded.')
            except OSError:
                raise Exception('use external hash enabled, but reward_hash.json not found.')
        else:
            self.all_graph_2_reward = {}
            print('external hash disabled.')
    ...
    def get_reward(self):
        if not self.current.graph_is_valid():
            ...
            self.hash_counter += 1
        elif sort_dict_string(self.current.graph) in self.all_graph_2_reward.keys():
            # this topology is found in the hash table stored in file
            # use the hash table, but still count the query number in this experiment
            topo_key = sort_dict_string(self.current.graph)
            self.graph_2_reward[topo_key] = self.all_graph_2_reward[topo_key]
            self.reward = self.graph_2_reward[topo_key][0]
            self.query_counter += 1

            self.seen_state_list.append(deepcopy(self.current))

            print('reward from hash table is ', self.reward)
        else:
            ...
            topo_key = sort_dict_string(self.current.graph)
            self.graph_2_reward[topo_key] = [self.reward, effi['efficiency'], effi['output_voltage']]
            self.all_graph_2_reward[topo_key] = self.graph_2_reward[topo_key]
