...
def serial_UCF_test(depth_list, trajectory, configs, date_str, Sim=None, uct_tree_list=None, keep_uct_tree=False):
        ...
        for num_runs in trajectory:
            ...
            for j in range(0, init_nums):
                ...
                for i in range(0, int(configs["game_num"] / init_nums)):
                    ...
                    # 0.03
                    # edges = [[0, 6], [1, 4], [2, 7], [3, 7], [5, 8]]
                    # 0.65
                    # edges = [[0, 8], [1, 4], [2, 6], [3, 5], [5, 7]]
                    edges = []
                    for edge in edges:
                    ...
                    while not sim.is_terminal():
                        ...
                        for n in range(configs["tree_num"]):
                            ...
                            tree_size_tmp, tree_tmp, depth = uct_tree_list[n].plan(False, sim.current.step - (
    ...
    return {'sim': uct_simulators[0],
            'time': (end_time - start_time).seconds,
            'query_num': total_query,
            # replace return the state list in the tree with all the seen states
            # 'state_list': uct_tree_list[0].get_all_states(),
            'state_list': uct_simulators[0].seen_state_list,
            'uct_tree': uct_tree,
