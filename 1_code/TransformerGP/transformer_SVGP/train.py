...
from dataset import get_loader, getRawData
...
def cal_loss(pred, gold, smoothing):
    ...
    if smoothing:
        ...
        loss = F.cross_entropy(pred, gold, ignore_index=Constants_PAD,
                               reduction='sum')  # This criterion combines log_softmax and nll_loss in a single function
...
def train_epoch(model, training_data, target, optimizer, device, smoothing=False, criterion=None, likelihood=None,
                gp=None):
...
def train(model, training_data, validation_data, optimizer, args, vocab, criterion, likelihood, gp):
    ...
    for epoch_i in range(args.epoch):
        ...
        train_loss, train_accu = train_epoch(
            model, training_data, args.target, optimizer, args.device, criterion=criterion, likelihood=likelihood,
            gp=gp)
        ...
        valid_loss, valid_accu = eval_epoch(model, validation_data, args.device, vocab, criterion=criterion,
                                            likelihood=likelihood, gp=gp)
...
def main(args=None, training_data=None, transformer=None, gp=None, likelihood=None, epoch=None, patience=None):
    """
    ...
    if args.data_train or training_data:
        ...
        early_stop = train(transformer, data_loader_training, data_loader_dev, optimizer, args, vocab, criterion, likelihood, gp)
    ...
    if args.data_train or training_data:
        return transformer, gp, likelihood, data_loader_training.dataset, early_stop
