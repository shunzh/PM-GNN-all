...
class Serial_GNN(nn.Module):
    def __init__(self, args):
        ...
        #                            n_edge_attr=args.len_edge_attr, n_layers=1, use_gpu=False,
        #                            dropout=args.dropout)
        #     self.gnn_encoders.append(self.gnn_encoder)
    ...
    def forward(self, input, return_edge_code=False):
        ...
        for i in range(gnn_layers):
            x2 = self.gnn_encoder2(x2, node_attr, edge_attr2, adj)
        # for i in range(gnn_layers):
        #     x1 = self.gnn_encoders[i](x1, node_attr, edge_attr1, adj)
        #     print(i)
        # x2 = x1
        # for i in range(gnn_layers):
        #     x2 = self.gnn_encoders[i+4](x2, node_attr, edge_attr2, adj)
        #     print(i+4)
...
class LOOP_GNN(nn.Module):
    ...
    def forward(self, input, return_edge_code=False):
        ...
        # for i in range(gnn_layers):
        #     x1 = self.gnn_encoders[i+i](x, node_attr, edge_attr1, adj)
        #     x = self.gnn_encoders[i+i+1](x1, node_attr, edge_attr2, adj)
